...
describe('get blog posts', () => {
  ...
  it('should return paginated view posts for concrete blog', async () => {
    ...
    const response = await public_postsTestHelper.getBlogPosts(blog.id);
  ...
  it('should support sorting by title asc and desc', async () => {
    ...
    const getAscSortedPostsResponse = await public_postsTestHelper.getBlogPosts(
      blog.id,
      {
        filter: {
    ...
    const getDescSortedPostsResponse = await public_postsTestHelper.getBlogPosts(
      blog.id,
      {
        filter: {
          sortBy: PostsSortBy.Title,
